    ...
    return <Dialog
        ...
        <DialogContent>
            <FormControl fullWidth>
                <RadioGroup
                    defaultValue={categories?.[0] !== undefined ? categories[0].name : ''}
                >
                    {
                        categories.map((c, index) => <Box key={index} sx={{ borderBottom: 1, borderColor: 'grey.500', py: 1 }}>
                            <FormControlLabel value={c.name} control={<Radio />} label={c.name} sx={{ width: 1 }} />
                        </Box>)
                    }
                </RadioGroup>
            </FormControl>
