...
const ChooseProvinceModal: React.FC<ChooseProvinceModalProps> = ({ open, setOpen }) => {
    ...
    const ProvinceSection = () => {
        return <>
            <Box sx={{ borderBottom: 1, borderColor: 'grey.500', py: 1 }}>
                <FormControlLabel value={'Toàn quốc'} control={<Radio />} label={'Toàn quốc'} sx={{ width: 1 }} />
            </Box>
            {
                provinces.map((p: any, index: number) => <Box key={index} sx={{ borderBottom: 1, borderColor: 'grey.500', py: 1 }}>
                    <FormControlLabel value={p.name} control={<Radio />} label={p.name} sx={{ width: 1 }}
                        onClick={() => {
    ...
    const DistrictSection = () => {
        return <>
            {
                districts.map((p: any, index: number) => <Box key={index} sx={{ borderBottom: 1, borderColor: 'grey.500', py: 1 }}>
                    <FormControlLabel value={p.name} control={<Radio />} label={p.name} sx={{ width: 1 }} onClick={() => { handleChangePage(p.code); }} />
                </Box>)
            }
        </>;
    };
        ...
        return <>
            {
                ...
                wards.map((p: any, index: number) => <Box key={index} sx={{ borderBottom: 1, borderColor: 'grey.500', py: 1 }}>
                    <FormControlLabel value={p.name} control={<Radio />} label={p.name} sx={{ width: 1 }} onClick={() => { handleChangePage(p.code); }} />
                </Box>)
            }
        </>;
    ...
    return <Dialog
        ...
        <DialogContent>
            <FormControl fullWidth>
                ...
                >
                    {
                        page === 0 ? <ProvinceSection /> :
                            page === 1 ? <DistrictSection /> :
                                page === 2 ? <WardSection /> : <></>
                    }
                </RadioGroup>
            </FormControl>
